...
# Ajuster un modèle linéaire initial avec la formule initiale
initial_model <- lmer(initial_formula, data = na.omit(data))
...
# On supprime age_FMR
new_formula <- update(initial_formula, . ~ . - age_FMR)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")


# On supprime solar_radiation_flight
new_formula <- update(new_formula, . ~ . - solar_radiation_flight)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")

# On supprime humidity_flight
new_formula <- update(new_formula, . ~ . - humidity_flight)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")

# On supprime pgi_1083
new_formula <- update(new_formula, . ~ . - Pgi_1083)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")


# On supprime pgi_331
new_formula <- update(new_formula, . ~ . - Pgi_331)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")


# On supprime FMR
new_formula <- update(new_formula, . ~ . - FMR)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")


# On supprime takeoff_time
new_formula <- update(new_formula, . ~ . - takeoff_time)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")

# On supprime mass
new_formula <- update(new_formula, . ~ . - mass)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")


# On supprime Pgi_105
new_formula <- update(new_formula, . ~ . - Pgi_105)  # Sélectionne toutes les variables sauf age_FMR
drop1(lmer(new_formula, data = na.omit(data)), test = "Chisq")
```


Tous les effets sont significatifs, mais toujours un problème de colinéarité avec l'effet aléatoire. Le modèle mixte n'est donc pas valide.

```{r}
mod_candidat <- lmer(new_formula, data = na.omit(data))
...
On obtient exactement le même modèle que précedemment car les valeurs de l'effet aléatoire sont = 0.

Le modèle est donc : 

mean_T_flight = 1.13 + 0.71 * air_T_flight - 1.74 * flight_duration + 1.79 * disturbed_flight[=='YES'] + 0.56 * takeoff_T_thorax


# CROSS VALIDATION DU MODELE ----------------------------------------------

On cherche à valider le pouvoir prédictif du modèle candidat avec la méthode de la cross validation en K-fold.

L'intérêt est de générer deux sous tableaux de données de tailles différentes : 'Entraînement' pour générer le modèle à partir ce ces données, et petit tableau 'Test' afin de tester la précision du modèle. Ce processus est effectué K fois et à chaque fois, un score de précision est calculé. 
Le nombre K est choisi en fonction de la taille du dataset. Par défaut, on choisit K = 10. 

Le résultat est une valeur de précision du modèle retenu (R²), les coefficients de chaque variable et l'errreur associée (RMSE et MAE).
...
print(model)
```
On obtient une précision de R² = 0.78 pour le modèle candidat.   
...
model$finalModel
```
 Le modele retenu par la validation croisee est 
mean_T_flight = 0.32 + 0.73 * air_T - 1.49 * flight_duration + 1.53 * disturbed_flight[==YES]+ 0.57 * takeoff_T_thorax

Avec 75 ddl, 71 residus. 

Les predictions pour chaque pli : 
```{r}
model$resample
```
