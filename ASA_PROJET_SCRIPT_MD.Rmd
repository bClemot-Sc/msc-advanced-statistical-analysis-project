...
Deux points semblent se démarquer dans les résidus. On les identifie :
```{r}
...
On cherche sur quel variable est le problème en représantant y ~ chque variable prédictive avec : rond plein : disturbed == YES et rond vide : disrturbed == NO
```{r, echo = F}
...
```
On teste la colinéarité de l'effet principal en l'intégrant comme effet fixe :
...
L'effet aléatoire est hautement multicolinéaire (d'où l'erreur "matrice singulière"). Il n'est pas intégrable dans le modèle, on revient donc à notre modèle initial à effets fixes.
