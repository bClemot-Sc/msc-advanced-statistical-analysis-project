...
library(corrplot)
library(ade4)
library(lubridate)  # Convertir donnÃ©es temporelles en secondes
library(car)  # Fonction vif() et anova()
library(lme4)  # ModÃ¨le gaussien mixte
library(missMDA)
library(plyr)
```
...
Le fichier de donnÃ©es possÃ¨de 32 variables mÃ©taboliques, gÃ©notypiques et environementales.
...
### --- Identification et visualisation des donnÃ©es manquantes
...
# SÃ©lection d'un seuil critique > 30%
pos_na_30 <- which(data_na > 30)

# On diffÃ©rentie les variables avec des couleurs selon le seuil critique
...
L'imputation des NAs est valable pour des valeurs entre 5 et 30% (Apporter des décisions avec la biblio), on choisira donc ici de ne supprimer aucune variables.
...
La fonction change le nom des levels, on va donc les réatribuer comme précédemment par soucis de clareté.
...
Les variables sun_flight et wind_flight ont une modalitÃ© ("1") avec respectivement, 1 et 2 rÃ©plicats. Ces modalitÃ©s sont alors inutilisables. On crÃ©Ã© donc une autre modalitÃ© : "\<2".
...
text(x = bar, y = table(data$sun_flight) - 1, label = table(data$sun_flight), pos = 3)  # Effectifs par catÃ©gorie

# wind_flight
bar <- barplot(table(data$wind_flight), main = "wind_flight", cex.main = .8, ylim = c(0, 60))
text(x = bar, y = table(data$wind_flight) - 1, label = table(data$wind_flight), pos = 3)  # Effectifs par catÃ©gorie
```
...
# IntÃ©ractions entre y \~ facteurs / co-variables

## A) GÃ¨ne \* environnement

### --- TempÃ©rature de l'air et SNP331 (ref : bibliographie)
...
plot(
  ...
  ylim = c(
    min(data$mean_T_flight) - 1,
    max(data$mean_T_flight  + 1)), ylab = "y = TÂ° moyenne", xlab = "Air"
    , main = "y ~ T air / SNP331"
  )
...
On observe pas de pattern clair d'interaction entre la tempÃ©rature de l'air et le SNP331 (tous les groupes [couleurs] de points se chevauchent) sur la tempÃ©rature du vol.
...
Les femelles sont nettement plus lourdes que les mÃ¢les ! **Forte intÃ©raction sexe x masse** sur la tempÃ©rature moyenne du corps pendant le vol.
...
plot(
  ...
  ylim = c(
    min(data$mean_T_flight) - 1,
    max(data$mean_T_flight  + 1)
  ), ylab = "y = TÂ° moyenne", xlab = "DurÃ©e de vol",
...
Il semble y avoir une faible interaction entre le sexe et la durÃ©e du vol.\
On observe aucune interaction entre durÃ©e du vol et sexe (simplement un effet du sexe).

### --- y \~ sexe\*tempÃ©rature au dÃ©collage
...
plot(
  ...
  ylim = c(
    min(data$mean_T_flight) - 1,
    max(data$mean_T_flight  + 1)
  ), ylab = "y = TÂ° moyenne", xlab = "TempÃ©rature au dÃ©collage",
  main = "y ~ Sexe / TÂ° dÃ©colage"
)
...
Pas d'interaction visible entre la tempÃ©rature au dÃ©collage et le sexe.
...
boxplot(
  ...
  ), ylab = "y = TÂ° moyenne", xlab = "SNP 331",
...
boxplot(
  ...
  ylim = c(
    min(data$takeoff_T_thorax) - 1,
    max(data$takeoff_T_thorax  + 1)
  ), ylab = "y = TÂ° moyenne", xlab = "SNP 105",
...
boxplot(
  ...
  ylim = c(
    min(data$takeoff_T_thorax) - 1,
    max(data$takeoff_T_thorax  + 1)
  ), ylab = "y = TÂ° moyenne", xlab = "SNP 1083",
...
On observe de claires interactions entre les variables gÃ©nÃ©tiques et le sexe du papillon sur la tempÃ©rature du corps.\

**Au vu des nombreuses interactions entre variables prÃ©dictives et sexes, nous avons dÃ©cidÃ© de mener les analyses en sÃ©parant les mÃ¢les et les femelles**.

## C) G?nes Hsp \* G?nes Pgi (bibliographie)
...
## y = température moyenne au vol SANS EFFET ALEATOIRES
...
```
On peut supprimer la variable Hsp70_1_134 car elle ne possède pas de coefficient significatif.
Voici notre nouveau modèle candidat.

```{r}
mod_candidat <- lm(formula = mean_T_flight ~ air_T + flight_duration + disturbed_flight + 
    takeoff_T_thorax + TnT2_100, data = data)
summary(mod_candidat)
```
```{r}
Anova(mod_candidat, type = "III")
```
Le **modèle candidat final** est donc le modèle ci-dessus.
La variable TnT2_100 n'est plus significative, on peut donc la retirer. 

```{r}
mod_candidat <- lm(formula = mean_T_flight ~ air_T + flight_duration + disturbed_flight + 
    takeoff_T_thorax, data = data)
summary(mod_candidat)
```
...
Les résidus sont distribués normalement, et aucun pattern ne semble émerger. Le graphique de Cooks ne montre la présence d'aucun poids aberrant parmi les résidus. Le modèle est donc interprétable !
...
model <- train(mean_T_flight ~ air_T + flight_duration + disturbed_flight + 
    takeoff_T_thorax + TnT2_100, data = data, 
...
print(model)
```
On obtient une précision de R² = 0.74 pour le modèle candidat.   
...
mean_T_flight = 3.04 + 0.73 * air_T - 1.49 * flight_duration + 1.53 * disturbed_flight[==YES]+ 0.57 * takeoff_T_thorax
